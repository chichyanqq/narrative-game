...
export default function App() {
  ...
  const callAPI = async (userMsg, currentMessages) => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: buildSystem({ world, protagonist, chars }),
          messages: newMsgs,
        }),
      });
      ...
      setStoryText("请求失败，请检查API key或网络。");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div style={{ width: 260, background: c.side, borderRight: `1px solid ${c.border}`, display: "flex", flexDirection: "column", flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
          {tab === "settings" && (
            <>
              {[
                { label: "API KEY", val: apiKey, set: setApiKey, ph: "sk-ant-...", rows: 2 },
              ...
              <button onClick={handleStart} style={{ width: "100%", padding: 10, background: c.accent, border: "none", borderRadius: 6, color: "#1a1714", fontFamily: "inherit", fontSize: 12, letterSpacing: "0.12em", cursor: "pointer", marginTop: 4 }}>
                开始游戏
              </button>
            </>
          )}
